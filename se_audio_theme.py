...
def configure_styles(style: ttk.Style) -> None:
    ...
    # Standard action button — orange text on panel, visible focus ring
...
def hero_button(parent, text: str, command) -> tk.Button:
    """The large orange-bordered primary action button."""
    btn = tk.Button(
...
def set_hero_enabled(btn, enabled, text=None):
    """Toggle a hero button between ready (orange) and disabled (muted).
...
BUG_REPORT_URL = "https://github.com/Godimas101/universal-audio-converter/issues/new"
...
class AudioEditorReferenceWindow(tk.Toplevel):
    ...
    _CONTENT = [
        ...
        ("op", "Space",                        "Play / pause."),
        ("op", "Ctrl + Z",                     "Undo the last edit."),
        ("op", "Ctrl + O",                     "Open a WAV file."),
        ("op", "Ctrl + S  ·  Ctrl + Shift + S", "Save  ·  Save As."),
        ("op", "Ctrl + A",                     "Select the whole clip."),
        ("op", "Esc",                          "Clear the current selection."),
        ("section", "CLIP"),
        ("op", "✂  TRIM",
    ...
    def __init__(self, parent):
        ...
        self.title("Audio Editor — Controls & Shortcuts")
    ...
    def _build(self):
        ...
        ttk.Label(hdr, text="▣  AUDIO EDITOR — CONTROLS & SHORTCUTS",
